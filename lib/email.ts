...
export async function sendInvoiceEmail(input: InvoiceEmailInput) {
  ...
  const payment = input.payId
    ? `<div style="margin:24px 0;padding:18px;border:1px solid #d9d9d9;background:#f7f7f5"><strong>Payment details</strong><br>PayID: ${escapeHtml(input.payId)}<br>Reference: ${escapeHtml(input.invoiceNumber)}</div>`
    : "";
  return sendEmail({
