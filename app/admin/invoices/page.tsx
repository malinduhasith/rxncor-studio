        ...
        {setupMissing ? (
          ...
          <>
            ...
            {selectedInvoice ? <section className={styles.invoiceDetail} aria-label={`${selectedInvoice.invoice_number} details`}>
              <div className={styles.detailHeading}><div><span>{selectedKind}</span><h3>{selectedInvoice.invoice_number}</h3><p>{selectedInvoice.client_name} · {selectedInvoice.project_title || "No project title"}</p></div><div className={styles.rowActions}><Link href={`/invoice/${selectedInvoice.public_token}`} target="_blank" rel="noreferrer"><ExternalLink size={14} /> Client view</Link><Link href="/admin/invoices">Close</Link></div></div>
