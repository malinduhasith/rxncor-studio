...
export async function createInvoiceAction(formData: FormData) {
  ...
  const {data:invoice,error}=await supabase.from("invoices").insert({...input,items:undefined,invoice_number:numberData,subtotal_cents:totals.subtotal,gst_cents:totals.gst,total_cents:totals.total,issuer_snapshot:{business_name:settings?.business_name||"RXNCOR Studio",issuer_name:settings?.issuer_name||"Malindu Herath",email:settings?.email,address:settings?.address,abn:settings?.abn},payment_snapshot:{pay_id:payId}}).select("id").single();
