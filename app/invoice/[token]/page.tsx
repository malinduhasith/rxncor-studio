    ...
    <main className={styles.shell}>
      ...
      <article className={styles.invoice}>
        ...
        <div className={styles.summaryArea}>
          ...
          <dl className={styles.totals}><dt>Subtotal</dt><dd>{aud(invoice.subtotal_cents)}</dd>{discountCents > 0 ? <><dt>Discount</dt><dd>−{aud(discountCents)}</dd></> : null}{invoice.gst_cents > 0 ? <><dt>GST ({invoice.gst_rate}%)</dt><dd>{aud(invoice.gst_cents)}</dd></> : null}<dt>Total AUD</dt><dd>{aud(invoice.total_cents)}</dd>{!isEstimate && totals.paid > 0 ? <><dt>Paid</dt><dd>−{aud(totals.paid)}</dd><dt className={styles.balance}>Balance</dt><dd className={styles.balance}>{aud(totals.balance)}</dd></> : null}{depositCents > 0 && totals.paid === 0 && !["draft", "void"].includes(invoice.status) ? <><dt className={styles.deposit}>Deposit requested</dt><dd className={styles.deposit}>{aud(depositCents)}</dd></> : null}</dl>
        </div>
