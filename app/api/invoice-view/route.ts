...
export async function POST(request: Request) {
  ...
  const { data: invoice } = await db.from("invoices").select("id,invoice_number,issuer_snapshot").eq("public_token", parsed.data.token).maybeSingle();
  if (!invoice) return NextResponse.json({ ok: true }, { headers: rateLimitHeaders() });
